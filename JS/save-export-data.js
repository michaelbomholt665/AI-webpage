...
function exportSavesJson() {
    ...
    try {
        if (!exportDataText) {
            ...
            newExportData.saves.forEach((save, index) => {
                if (!save.number || !save.description || !save.files) {
                    throw new Error(`Save #${index + 1} is missing required properties.`);
                }
            });
        }
    } catch (error) {
...
// Placeholder for exportSavesManually (requires autosave.js logic)
function exportSavesManually() {
    alert("Export Saves Manually not implemented yet. Define in autosave.js.");
}
